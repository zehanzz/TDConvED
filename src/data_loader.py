...
class msr_vtt_dataset(Dataset):
	def __init__(self,vocab_file,image_dir,split,batch,shuffle=False):
		with open(vocab_file,"r") as f:
			vocab=json.load(f)
		if 'ref' in vocab.keys():
			print("'ref' exists in vocab!")
		else:
    			print("'ref' doesn't exist in vocab!")

		print(len(vocab))
		self.batch=batch
		self.split=os.path.join(image_dir,split)
		print("Constructed path:", self.split)
		self.ref=vocab['ref']
		self.w2i=vocab['w2i']
		self.i2w=vocab['i2w']
		...
		if shuffle==True:
			...
			if len(combo) == 0:
    				raise ValueError("combo is empty!")
...
if __name__=="__main__":
	msr_vtt=msr_vtt_dataset("../data/msr_vtt/train_vocab.json","../data/msr_vtt","train",17)
	print(msr_vtt[5])
	print(msr_vtt[-1])
